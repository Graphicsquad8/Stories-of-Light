...
export function AdSlot({ slot, className = "", label, disabled, contentId, contentType, contentManualMode }: AdSlotProps) {
  ...
  if (!platform || !code) {
    if (label !== undefined) {
      return (
        <div
          className={`relative flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 text-center gap-1 ${className}`}
          style={{ minHeight: "inherit" }}
          data-ad-slot={slot}
          data-testid={`ad-placeholder-${label.replace(/\s+/g, "-").toLowerCase()}`}
        >
          <span className="text-[10px] uppercase tracking-widest font-semibold text-muted-foreground/60 select-none">
            Advertisement
          </span>
          <span className="text-xs text-muted-foreground/80 font-medium select-none">
            {label}
          </span>
        </div>
      );
