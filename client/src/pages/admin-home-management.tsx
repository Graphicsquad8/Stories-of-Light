          ...
          {section.id === "categories" && categories && categories.length > 0 && (
            <div className="space-y-2">
              <Label className="text-xs flex items-center gap-1.5">
                ...
                <span className="text-muted-foreground ml-1">(uncheck to hide)</span>
              </Label>
                ...
                {categories.map(cat => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[9px] text-muted-foreground/60 shrink-0 capitalize">{cat.type?.replace("-", " ")}</span>
                    </label>
                  );
                })}
              </div>
...
export default function AdminHomeManagementPage() {
  ...
  useEffect(() => {
    ...
    setHeroImage(settings["homeHeroBgImage"] || "");
  ...
  const saveSetting = useCallback(async (key: string, value: string) => {
    ...
    queryClient.removeQueries({ queryKey: ["/api/settings/public"] });
  }, []);
  ...
  const heroMutation = useMutation({
    mutationFn: async () => {
      await Promise.all([
        ...
        saveSetting("homeHeroBgImage", heroImage),
      ]);
    },
    onSuccess: () => {
  ...
  const handleHeroImageUpload = async (file: File) => {
    ...
    try {
      ...
      setHeroImage(newUrl);
      await saveSetting("homeHeroBgImage", newUrl);
  ...
  const handleHeroImageDelete = async () => {
    setHeroImage("");
    await saveSetting("homeHeroBgImage", "");
    toast({ title: "Hero image removed", description: "Default image will be used." });
  };
            ...
            <Card className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 bg-black/30 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        ...
                        <Button
                          ...
                          data-testid="button-delete-hero-image"
                        >
                    ...
                    >
                      {heroImageUploading ? (
                        ...
                        <>
                          ...
                          <span className="text-[11px] text-muted-foreground/70">JPG, PNG, WebP — recommended 1920×600</span>
                        </>
                      )}
                    </button>
                  )}
                ...
                {heroImage && (
                  <div className="flex gap-2">
                    <Button
                      ...
                      disabled={heroImageUploading}
                      className="text-xs"
                      data-testid="button-change-hero-image"
                    >
                      {heroImageUploading ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Upload className="w-3.5 h-3.5 mr-1.5" />}
                      Change Image
                    </Button>
                    <Button
                      size="sm"
                ...
                <p className="text-xs text-muted-foreground">
                  Changes apply immediately after upload. The default image is used when no custom image is set.
                </p>
              </div>
