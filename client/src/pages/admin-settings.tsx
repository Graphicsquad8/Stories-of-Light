...
import {
  ...
  ToggleLeft, LogIn, Shield, KeyRound, Lock, Code2, Copy, Check,
  Bot,
} from "lucide-react";
...
function GoogleOAuthSection({
  ...
  const autoSavedRedirect = useRef(false);

  const autoRedirectUrl = typeof window !== "undefined"
  ...
  useEffect(() => {
    ...
    const saved = settings.googleRedirectUrl;
    setRedirectUrl(saved || autoRedirectUrl);
    if (!saved && autoRedirectUrl && !autoSavedRedirect.current) {
      autoSavedRedirect.current = true;
      onSave("googleRedirectUrl", autoRedirectUrl);
    }
  }, [settings]);

  useEffect(() => {
    if (prevSaving.current === "googleClientSecret" && saving === null) {
      setClientSecret("");
      setShowSecret(false);
      toast({ title: "Client secret saved & encrypted" });
    }
      ...
      <div className="space-y-4 border rounded-lg p-4 bg-muted/20">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={saving === "googleClientId" || !clientId}
              title="Save"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="google-client-secret">
            ...
            {hasSecret && (
              <Badge variant="outline" className="ml-2 text-xs font-normal text-green-700 dark:text-green-400 border-green-300 dark:border-green-700">
                <Check className="w-2.5 h-2.5 mr-1" />Saved & encrypted
              </Badge>
            )}
          </Label>
          <p className="text-xs text-muted-foreground">
            {hasSecret ? "Enter a new value only to update the secret. Leave blank to keep existing." : "Paste your Google Client Secret from Google Cloud Console."}
          </p>
          <div className="flex gap-2">
            <div className="relative flex-1">
              <Input
            ...
            <Button
              size="icon"
              onClick={() => { if (clientSecret) onSave("googleClientSecret", clientSecret); }}
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            This URL is auto-generated. Copy it and add it to your Google Cloud Console → OAuth 2.0 → Authorized redirect URIs.
          </p>
          <div className="flex gap-2">
            <Input
            ...
            <Button
              ...
              title="Copy URL"
              data-testid="button-copy-google-redirect-url"
            >
        ...
        <div className="mt-3 p-3 rounded-md bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 text-xs text-blue-700 dark:text-blue-300 space-y-1">
          ...
          <ol className="list-decimal list-inside space-y-0.5 text-blue-600 dark:text-blue-400">
            ...
            <li>Paste the Authorized Redirect URL above into "Authorized redirect URIs"</li>
            <li>Copy the Client ID and Client Secret into the fields above and save each</li>
            <li>Enable the toggle above to activate Google Login</li>
          </ol>
        </div>
      </div>
    </div>
    ...
    <AdminLayout>
      <div className="space-y-3 max-w-3xl">
        ...
        <Link href="/image/api-generator">
          <div
            className="w-full flex items-center justify-between border-2 border-primary/30 bg-primary/5 hover:bg-primary/10 hover:border-primary/60 transition-all rounded-xl px-5 py-4 cursor-pointer group"
        ...
        <Link href="/image/ai-integration">
          <div
            ...
            data-testid="button-ai-integration"
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
              ...
              <div className="text-left">
                <p className="font-semibold text-sm text-foreground">AI Model Integration</p>
                <p className="text-xs text-muted-foreground">Connect AI models, manage system prompts, and configure webhook automation</p>
              </div>
            </div>
            <ChevronRight className="w-5 h-5 text-muted-foreground group-hover:text-primary transition-colors shrink-0" />
          </div>
