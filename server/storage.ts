import {
  ...
  manualAds, apiKeys,
  aiModels, aiConversations, aiMessages,
} from "@shared/schema";
import { eq, desc, and, or, ilike, sql, count, sum, inArray, notInArray, asc, gte, lte, ne, isNull, isNotNull } from "drizzle-orm";
...
export interface IStorage {
  ...
  getStories(opts?: { status?: string; categoryId?: string; featured?: boolean; search?: string; limit?: number; offset?: number; userId?: string; includeNullUser?: boolean; startDate?: string; endDate?: string; sortBy?: "views" | "date"; activeOnly?: boolean }): Promise<StoryWithCategory[]>;
  ...
  getBooks(opts?: { type?: string; category?: string; search?: string; sort?: string; minRating?: number; userId?: string; published?: boolean; activeOnly?: boolean }): Promise<Book[]>;
  ...
  getMotivationalStories(opts?: { category?: string; search?: string; sort?: string; published?: boolean; limit?: number; offset?: number; userId?: string; includeNullUser?: boolean; startDate?: string; endDate?: string; activeOnly?: boolean }): Promise<{ stories: MotivationalStory[]; total: number }>;
  ...
  getDuas(opts?: { published?: boolean; search?: string; category?: string; sort?: string; limit?: number; offset?: number; userId?: string; includeNullUser?: boolean; startDate?: string; endDate?: string; activeOnly?: boolean }): Promise<{ duas: Dua[]; total: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getStories(opts?: { status?: string; categoryId?: string; featured?: boolean; search?: string; limit?: number; offset?: number; userId?: string; includeNullUser?: boolean; startDate?: string; endDate?: string; sortBy?: "views" | "date"; activeOnly?: boolean }): Promise<StoryWithCategory[]> {
  ...
  async getBooks(opts?: { type?: string; category?: string; search?: string; sort?: string; minRating?: number; userId?: string; published?: boolean; activeOnly?: boolean }): Promise<Book[]> {
    ...
    if (opts?.activeOnly) conditions.push(eq(books.isActive, true));
  ...
  async getMotivationalStories(opts?: { category?: string; search?: string; sort?: string; published?: boolean; limit?: number; offset?: number; userId?: string; includeNullUser?: boolean; startDate?: string; endDate?: string; activeOnly?: boolean }): Promise<{ stories: MotivationalStory[]; total: number }> {
    ...
    if (opts?.activeOnly) conditions.push(eq(motivationalStories.isActive, true));
  ...
  async getDuas(opts: { published?: boolean; search?: string; category?: string; sort?: string; limit?: number; offset?: number; userId?: string; includeNullUser?: boolean; startDate?: string; endDate?: string; activeOnly?: boolean } = {}): Promise<{ duas: Dua[]; total: number }> {
    ...
    if (opts.activeOnly) conditions.push(eq(duas.isActive, true));
    const where = and(...conditions);
    let orderClause;
  ...
  async listAiModels(): Promise<AiModel[]> {
    return db.select().from(aiModels).orderBy(asc(aiModels.createdAt));
  }
  ...
  async getDefaultAiModel(type: string): Promise<AiModel | undefined> {
    const [row] = await db.select().from(aiModels).where(and(eq(aiModels.type, type), eq(aiModels.isDefault, true), eq(aiModels.isActive, true)));
    if (row) return row;
    const [fallback] = await db.select().from(aiModels).where(and(eq(aiModels.type, type), eq(aiModels.isActive, true)));
    return fallback;
  }

  async createAiModel(data: InsertAiModel): Promise<AiModel> {
    const [row] = await db.insert(aiModels).values(data).returning();
    return row;
  }

  async updateAiModel(id: string, data: Partial<InsertAiModel>): Promise<AiModel | undefined> {
    const [row] = await db.update(aiModels).set(data).where(eq(aiModels.id, id)).returning();
    return row;
  }
  ...
  async listAiConversations(userId: string): Promise<AiConversation[]> {
    const withMessages = db.select({ id: aiMessages.conversationId }).from(aiMessages).groupBy(aiMessages.conversationId);
    return db.select().from(aiConversations)
      .where(and(eq(aiConversations.userId, userId), inArray(aiConversations.id, withMessages)))
      .orderBy(desc(aiConversations.updatedAt));
  }

  async deleteEmptyAiConversations(userId: string): Promise<void> {
    const withMessages = db.select({ id: aiMessages.conversationId }).from(aiMessages).groupBy(aiMessages.conversationId);
    await db.delete(aiConversations).where(and(eq(aiConversations.userId, userId), notInArray(aiConversations.id, withMessages)));
  }

  async getAiConversation(id: string): Promise<(AiConversation & { messages: AiMessage[] }) | undefined> {
  ...
  async createAiConversation(userId: string, title?: string): Promise<AiConversation> {
    const [row] = await db.insert(aiConversations).values({ userId, title: title || "New Conversation" }).returning();
    return row;
  }

  async updateAiConversation(id: string, data: { title?: string; updatedAt?: Date }): Promise<AiConversation | undefined> {
    const [row] = await db.update(aiConversations).set({ ...data, updatedAt: new Date() }).where(eq(aiConversations.id, id)).returning();
    return row;
  }
  ...
  async addAiMessage(data: { conversationId: string; role: string; content: string; imageUrl?: string; modelId?: string }): Promise<AiMessage> {
  ...
  async toggleAiMessageBookmark(id: string): Promise<AiMessage | undefined> {
    const [msg] = await db.select().from(aiMessages).where(eq(aiMessages.id, id));
    if (!msg) return undefined;
    const [row] = await db.update(aiMessages).set({ bookmarked: !msg.bookmarked }).where(eq(aiMessages.id, id)).returning();
    return row;
  }

  async getBookmarkedAiMessages(conversationId: string): Promise<AiMessage[]> {
    return db.select().from(aiMessages).where(and(eq(aiMessages.conversationId, conversationId), eq(aiMessages.bookmarked, true))).orderBy(asc(aiMessages.createdAt));
  }
}
