  ...
  app.get("/api/admin/dashboard", requireStaff, async (_req, res) => {
    try {
      ...
      const [usersRes, topStoriesRes, topDuasRes, topBooksRes, topMotivRes,
        ...
        pool.query(`SELECT id, title, views, average_rating, category FROM books WHERE deleted_at IS NULL ORDER BY views DESC, average_rating DESC NULLS LAST LIMIT 5`),
        pool.query(`SELECT id, title, views, average_rating, category FROM motivational_stories WHERE deleted_at IS NULL ORDER BY views DESC, average_rating DESC NULLS LAST LIMIT 5`),
