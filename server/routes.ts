  ...
  app.get("/api/admin/dashboard", requireStaff, async (_req, res) => {
    try {
      ...
      const [usersRes, topStoriesRes, topDuasRes, topBooksRes, topMotivRes,
        ...
        pool.query(`SELECT s.id, s.title, s.excerpt, COALESCE(COUNT(b.id), 0) as views, c.name as category_name FROM stories s LEFT JOIN categories c ON s.category_id = c.id LEFT JOIN bookmarks b ON b.story_id = s.id WHERE s.deleted_at IS NULL GROUP BY s.id, s.title, s.excerpt, c.name ORDER BY views DESC LIMIT 5`),
        pool.query(`SELECT id, title, description, views, category FROM duas WHERE deleted_at IS NULL ORDER BY views DESC LIMIT 5`),
        pool.query(`SELECT id, title, description, views, average_rating, category FROM books WHERE deleted_at IS NULL ORDER BY views DESC, average_rating DESC NULLS LAST LIMIT 5`),
        pool.query(`SELECT id, title, description, views, average_rating, category FROM motivational_stories WHERE deleted_at IS NULL ORDER BY views DESC, average_rating DESC NULLS LAST LIMIT 5`),
        pool.query(`SELECT s.id, s.title, s.excerpt as description, COUNT(b.id) as bookmark_count FROM stories s JOIN bookmarks b ON b.story_id = s.id WHERE s.deleted_at IS NULL GROUP BY s.id, s.title, s.excerpt ORDER BY bookmark_count DESC LIMIT 5`),
        pool.query(`SELECT d.id, d.title, d.description, COUNT(db.id) as bookmark_count FROM duas d JOIN dua_bookmarks db ON db.dua_id = d.id WHERE d.deleted_at IS NULL GROUP BY d.id, d.title, d.description ORDER BY bookmark_count DESC LIMIT 5`),
        pool.query(`SELECT b.id, b.title, b.description, COUNT(bb.id) as bookmark_count FROM books b JOIN book_bookmarks bb ON bb.book_id = b.id WHERE b.deleted_at IS NULL GROUP BY b.id, b.title, b.description ORDER BY bookmark_count DESC LIMIT 5`),
        pool.query(`SELECT ms.id, ms.title, ms.description, COUNT(mb.id) as bookmark_count FROM motivational_stories ms JOIN motivational_bookmarks mb ON mb.story_id = ms.id WHERE ms.deleted_at IS NULL GROUP BY ms.id, ms.title, ms.description ORDER BY bookmark_count DESC LIMIT 5`),
        ...
        pool.query(`SELECT s.id, s.title, s.excerpt as description, s.status, s.updated_at, c.name as category_name FROM stories s LEFT JOIN categories c ON s.category_id = c.id WHERE s.deleted_at IS NULL ORDER BY s.updated_at DESC LIMIT 8`),
        pool.query(`SELECT id, title, description, updated_at, CASE WHEN published THEN 'published' ELSE 'draft' END as status, category as category_name FROM duas WHERE deleted_at IS NULL ORDER BY updated_at DESC LIMIT 8`),
