...
export async function registerRoutes(
  ...
  app.get("/api/admin/ai-integration-settings", requireAdmin, async (_req, res) => {
    const [enabled, systemMessage, textWebhookUrl, imageWebhookUrl] = await Promise.all([
      storage.getSetting("ai_integration_enabled"),
      storage.getSetting("ai_integration_system_message"),
      storage.getSetting("ai_integration_text_webhook"),
      storage.getSetting("ai_integration_image_webhook"),
    ]);
    res.json({
      enabled: enabled !== "false",
      systemMessage: systemMessage || "",
      textWebhookUrl: textWebhookUrl || "",
      imageWebhookUrl: imageWebhookUrl || "",
    });
  });

  app.post("/api/admin/ai-integration-settings", requireAdmin, async (req, res) => {
    const { enabled, systemMessage, textWebhookUrl, imageWebhookUrl } = req.body;
    await Promise.all([
      storage.setSetting("ai_integration_enabled", String(enabled !== false)),
      storage.setSetting("ai_integration_system_message", systemMessage || ""),
      storage.setSetting("ai_integration_text_webhook", textWebhookUrl || ""),
      storage.setSetting("ai_integration_image_webhook", imageWebhookUrl || ""),
    ]);
    res.json({ ok: true });
  });

  // Public read for AI integration status (used by AI Studio from staff)
  app.get("/api/admin/ai-integration-status", requireStaff, async (_req, res) => {
    const [enabled, systemMessage, textWebhookUrl, imageWebhookUrl] = await Promise.all([
      storage.getSetting("ai_integration_enabled"),
      storage.getSetting("ai_integration_system_message"),
      storage.getSetting("ai_integration_text_webhook"),
  ...
  app.get("/api/admin/ai-models", requireStaff, async (_req, res) => {
    const models = await storage.listAiModels();
    const masked = models.map(m => ({ ...m, apiKey: m.apiKey ? "••••••••" : "" }));
    res.json(masked);
  });

  app.post("/api/admin/ai-models", requireAdmin, async (req, res) => {
    try {
      const { name, provider, modelId, type, apiKey, baseUrl, isActive, isDefault } = req.body;
      if (!name || !provider || !modelId || !type || !apiKey) return res.status(400).json({ message: "Missing required fields" });
      const encrypted = encryptSecret(apiKey);
      const model = await storage.createAiModel({ name, provider, modelId, type, apiKey: encrypted, baseUrl: baseUrl || null, isActive: isActive !== false, isDefault: !!isDefault });
  ...
  app.patch("/api/admin/ai-models/:id", requireAdmin, async (req, res) => {
    ...
    res.json({ ...model, apiKey: "••••••••" });
  });

  app.delete("/api/admin/ai-models/:id", requireAdmin, async (req, res) => {
    const ok = await storage.deleteAiModel(req.params.id);
    if (!ok) return res.status(404).json({ message: "Not found" });
    res.json({ success: true });
  });

  // ─── AI Conversations (Staff only) ────────────────────────────────────────

  app.get("/api/admin/ai-conversations", requireStaff, async (req, res) => {
    const userId = (req.user as any).id;
    await (storage as any).deleteEmptyAiConversations(userId);
    const convs = await storage.listAiConversations(userId);
    res.json(convs);
  });

  app.post("/api/admin/ai-conversations", requireStaff, async (req, res) => {
    const conv = await storage.createAiConversation((req.user as any).id, req.body.title);
    res.status(201).json(conv);
  });

  app.get("/api/admin/ai-conversations/:id", requireStaff, async (req, res) => {
    const conv = await storage.getAiConversation(req.params.id);
    if (!conv) return res.status(404).json({ message: "Not found" });
    res.json(conv);
  });

  app.patch("/api/admin/ai-conversations/:id", requireStaff, async (req, res) => {
    const conv = await storage.updateAiConversation(req.params.id, { title: req.body.title });
    if (!conv) return res.status(404).json({ message: "Not found" });
    res.json(conv);
  });

  app.delete("/api/admin/ai-conversations/:id", requireStaff, async (req, res) => {
    const ok = await storage.deleteAiConversation(req.params.id);
    if (!ok) return res.status(404).json({ message: "Not found" });
    res.json({ success: true });
  });

  app.post("/api/admin/ai-messages/:id/bookmark", requireStaff, async (req, res) => {
    const msg = await storage.toggleAiMessageBookmark(req.params.id);
  ...
  function extractAiError(rawText: string): string {
    const trimmed = rawText.trim();
    // HTML error page (e.g. wrong base URL or network error)
    if (trimmed.startsWith("<") || trimmed.startsWith("<!")) {
      return "The AI provider returned an HTML page instead of JSON. Please check the Base URL and Model ID in Settings → AI Models.";
    }
    try {
      const parsed = JSON.parse(trimmed);
      const obj = Array.isArray(parsed) ? parsed[0] : parsed;
      const msg = obj?.error?.message || obj?.error?.error || obj?.message || obj?.detail || obj?.error || null;
      if (msg && typeof msg === "string") return msg.replace(/\n/g, " ").trim();
    } catch {}
    // Return cleaned text, no HTML tags
    return trimmed.replace(/<[^>]+>/g, "").slice(0, 300).trim() || "AI request failed";
  }

  app.post("/api/admin/ai-chat", requireStaff, async (req, res) => {
    try {
      const { modelId, messages: rawMessages, conversationId } = req.body as { modelId?: string; messages: { role: string; content: string }[]; conversationId?: string };
      if (!rawMessages?.length) return res.status(400).json({ message: "messages required" });
      // Strip any local-only roles (e.g. "error") that AI providers don't accept
      const messages = rawMessages.filter(m => ["system", "user", "assistant"].includes(m.role));

      let model: any;
      if (modelId) {
        model = await storage.getAiModel(modelId);
      } else {
        model = await storage.getDefaultAiModel("text");
      }
      if (!model) return res.status(400).json({ message: "No text AI model configured. Please add one in Settings → AI Models." });

      const apiKey = decryptSecret(model.apiKey);
      const baseUrl = model.baseUrl || (
        model.provider === "openai" ? "https://api.openai.com/v1" :
        model.provider === "anthropic" ? "https://api.anthropic.com/v1" :
        model.provider === "google" ? "https://generativelanguage.googleapis.com/v1beta/openai" :
        "https://api.openai.com/v1"
      );
      // Google requires bare model name like "gemini-1.5-pro" not "models/gemini-1.5-pro"
      const normalizedModelId = model.provider === "google"
        ? model.modelId.replace(/^models\//, "").replace(/^gemini-/, "gemini-")
        : model.modelId;

      let responseText = "";

      if (model.provider === "anthropic") {
        const fetchRes = await fetch(`${baseUrl}/messages`, {
          method: "POST",
          headers: { "x-api-key": apiKey, "anthropic-version": "2023-06-01", "content-type": "application/json" },
          body: JSON.stringify({ model: normalizedModelId, max_tokens: 4096, messages: messages.filter(m => m.role !== "system"), system: messages.find(m => m.role === "system")?.content }),
        });
        if (!fetchRes.ok) { const raw = await fetchRes.text(); throw new Error(extractAiError(raw)); }
        ...
        const fetchRes = await fetch(`${baseUrl}/chat/completions`, {
          ...
          body: JSON.stringify({ model: normalizedModelId, messages, max_tokens: 4096 }),
        });
        if (!fetchRes.ok) { const raw = await fetchRes.text(); throw new Error(extractAiError(raw)); }
        const data: any = await fetchRes.json();
      ...
      if (conversationId) {
        ...
        if (lastUserMsg) await storage.addAiMessage({ conversationId, role: "user", content: lastUserMsg.content, modelId: model.id });
        await storage.addAiMessage({ conversationId, role: "assistant", content: responseText, modelId: model.id });
        if (userMessages.length <= 1) {
          const shortTitle = lastUserMsg?.content.slice(0, 60) || "New Conversation";
          await storage.updateAiConversation(conversationId, { title: shortTitle });
        }
      }

      res.json({ content: responseText, modelName: model.name });
    } catch (e: any) {
      console.error("AI chat error:", e.message);
      res.status(500).json({ message: e.message || "AI request failed" });
    }
  });

  // ─── AI Image Generation (proxy) ──────────────────────────────────────────

  app.post("/api/admin/ai-image", requireStaff, async (req, res) => {
    try {
      const { modelId, prompt, n = 2, size = "1024x1024" } = req.body as { modelId?: string; prompt: string; n?: number; size?: string };
      ...
      if (modelId) {
        ...
        model = await storage.getDefaultAiModel("image");
      }
      if (!model) return res.status(400).json({ message: "No image AI model configured. Please add one in Settings → AI Models." });
      ...
      if (model.provider === "stability") {
        const fetchRes = await fetch("https://api.stability.ai/v1/generation/stable-diffusion-xl-1024-v1-0/text-to-image", {
          method: "POST",
          headers: { "Authorization": `Bearer ${apiKey}`, "content-type": "application/json", "Accept": "application/json" },
          body: JSON.stringify({ text_prompts: [{ text: prompt, weight: 1 }], cfg_scale: 7, height: 1024, width: 1024, steps: 30, samples: Math.min(n, 4) }),
        });
        if (!fetchRes.ok) { const raw = await fetchRes.text(); throw new Error(extractAiError(raw)); }
        const data: any = await fetchRes.json();
        urls = (data.artifacts || []).map((a: any) => `data:image/png;base64,${a.base64}`);
      } else {
        const fetchRes = await fetch(`${baseUrl}/images/generations`, {
          method: "POST",
          headers: { "Authorization": `Bearer ${apiKey}`, "content-type": "application/json" },
          body: JSON.stringify({ model: model.modelId, prompt, n: Math.min(n, 4), size, response_format: "url" }),
        });
        if (!fetchRes.ok) { const raw = await fetchRes.text(); throw new Error(extractAiError(raw)); }
        const data: any = await fetchRes.json();
  ...
  app.post("/api/admin/ai-publish", requireStaff, async (req, res) => {
    try {
      const { title, content, excerpt, categoryId, contentType, thumbnail } = req.body as { title: string; content: string; excerpt?: string; categoryId?: string; contentType: string; thumbnail?: string };
      if (!title || !content || !contentType) return res.status(400).json({ message: "title, content, and contentType are required" });

      const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") + "-" + Date.now().toString(36);

      if (contentType === "story") {
        const story = await storage.createStory({ title, slug, content, excerpt: excerpt || "", categoryId: categoryId || null, thumbnail: thumbnail || null, published: false, tags: [] } as any);
        return res.status(201).json({ type: "story", id: story.id, slug: story.slug });
      }
      if (contentType === "motivational-story") {
        const ms = await storage.createMotivationalStory({ title, slug, content, description: excerpt || "", published: false } as any);
        return res.status(201).json({ type: "motivational-story", id: ms.id, slug: ms.slug });
      }
      if (contentType === "dua") {
        const dua = await storage.createDua({ title, slug, description: excerpt || "", published: false } as any);
        return res.status(201).json({ type: "dua", id: dua.id, slug: dua.slug });
      }
      if (contentType === "book") {
        const book = await storage.createBook({ title, slug, description: excerpt || "", published: false } as any);
        return res.status(201).json({ type: "book", id: book.id, slug: book.slug });
      }
      return res.status(400).json({ message: "Unknown content type" });
    } catch (e: any) {
      console.error("AI publish error:", e.message);
