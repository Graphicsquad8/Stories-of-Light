  ...
  app.get("/api/admin/dashboard", requireStaff, async (_req, res) => {
    try {
      ...
      const [usersRes, topStoriesRes, topDuasRes, topBooksRes, topMotivRes,
        ...
        pool.query(`SELECT s.id, s.title, s.excerpt, COUNT(b.id) as bookmark_count FROM stories s JOIN bookmarks b ON b.story_id = s.id WHERE s.deleted_at IS NULL GROUP BY s.id, s.title, s.excerpt ORDER BY bookmark_count DESC LIMIT 5`),
        ...
        pool.query(`SELECT s.id, s.title, s.excerpt, s.status, s.updated_at, c.name as category_name FROM stories s LEFT JOIN categories c ON s.category_id = c.id WHERE s.deleted_at IS NULL ORDER BY s.updated_at DESC LIMIT 8`),
