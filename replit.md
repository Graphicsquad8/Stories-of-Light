...
- **Home Page Management (Dialog from Categories)**: Removed from the sidebar nav. Accessed via a "Home Page Management" button at the top of the Categories admin page (`/image/categories`). Opens as a Dialog with two expandable box-style panels: Hero Section (title, subtitle, btn1, btn2) and Page Section (fixed layout: Ad Top → Explore by Category → Featured Stories → Islamic Books → Ad Mid → Motivational Stories → Duas → Latest Stories → Ad Bottom). Each content section has editable title/desc, visibility toggle, count control, and optional manual category selection (for the categories section). Ad slots open the existing AdManagementDialog with new content types `home-top`, `home-mid`, `home-bottom`. Slot modes stored as settings keys `homeAdSlot_top/mid/bottom`. Backend route: `PATCH /api/admin/home-page/:slotId/ad-slots`.
- **Home Page Fixed Layout**: The home page (`home.tsx`) now follows a fixed render order: Hero → Ad Top → Categories → Featured → Books → Ad Mid → Motivational → Duas → Latest → Ad Bottom → Newsletter. Previously dynamic (split topHalf/bottomHalf). Each section reads its title/desc from `homeSectionsConfig` settings. Category section filters tiles by `categoryIds` if specified. Books section renamed from "Free Islamic Books" to "Islamic Books" (driven by admin-editable title).
- **Section Settings Extended**: `homeSectionsConfig` JSON now stores `title`, `desc`, and `categoryIds` per section alongside `id`, `visible`, `count`. Both admin and frontend parse with fallback to defaults per section ID.
